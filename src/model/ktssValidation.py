...
class KTSSValidator(AbstractValidationArguments):
    ...
    **generate_distances** method """
    ...
    def _get_next_state(self, sequence: str, state: bool = False) -> str:
    ...
    def generate_sequence(
        ...
        and the method gets the symbols list `["a"]` and the state `"a"`, the method
        will return the sequences:
    ...
    def _generate_infixes(
        ...
        """
        prefix_state = self._get_next_state(prefix)
        sequences = self.generate_sequence(self.infix_symbols, state=prefix_state)

        results = []
        for sequence in sequences:
            state = self._get_next_state(sequence, state=prefix_state)

            if not state:
                continue

            try:
                self._get_next_state(suffix, state=state)
    ...
    @staticmethod
    def _get_minimum_distances(distances: dict) -> dict:
    ...
    def generate_distances(
        ...
        for sequence_raw in tqdm(sequences, file=tqdm_out):
            ...
            try:
                infix_sequences = self._generate_infixes(
            ...
            for infix_sequence in infix_sequences:
                ...
                distance = self._string_distances(infix, infix_string)
    ...
    def annotate_sequence(self):
        """
        Gets a string sequence, and analize it symbol to symbol. When a mutation symbol
        is found, it starts analyzing the mutation symbol and when it finish, continues
        with the last symbols
        """
